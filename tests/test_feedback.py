...
def test_un_motivo_muy_largo_se_trunca(sesion):
    ...
    sesion.add(Decision(job_id=job.id, estado="interesa", motivo="x" * 5000))
    sesion.commit()
...
def test_el_conjunto_de_ejemplos_no_supera_el_presupuesto(sesion):
    for i in range(6):
        job = crea_job(sesion, f"p{i}")
        sesion.add(Decision(job_id=job.id, estado="interesa", motivo="me gusta " * 200))
    for i in range(6):
        job = crea_job(sesion, f"n{i}")
        sesion.add(Decision(job_id=job.id, estado="descartada", motivo="no me gusta " * 200))
    sesion.commit()
...
def test_el_equilibrio_se_mantiene_con_motivos_largos(sesion):
    for i in range(6):
        job = crea_job(sesion, f"p{i}")
        sesion.add(Decision(job_id=job.id, estado="interesa", motivo="me gusta " * 100))
    for i in range(2):
        job = crea_job(sesion, f"n{i}")
        sesion.add(Decision(job_id=job.id, estado="descartada", motivo="no me gusta " * 100))
    sesion.commit()
